...
local IPK_Version="20200422.2.11"
local m, s, o
local redir_run=0
local reudp_run=0
local sock5_run=0
...
if gfwmode==1 then 
 ...
 if nixio.fs.access("/etc/dnsmasq.firefly/ad.conf") then
  ad_count=tonumber(sys.exec("cat /etc/dnsmasq.firefly/ad.conf | wc -l"))
 end
end
 
if nixio.fs.access("/etc/china_cdn.txt") then 
 ip_count = sys.exec("cat /etc/china_cdn.txt | wc -l")
end
